...
def generate_launch_description():
    ...
    pkg_bt = get_package_share_directory('team2_nav')

    # Define nav_to_pose behaviour tree
    bt_xml_navtopose_file = PathJoinSubstitution([pkg_bt, 'behavior_tree_xml', 'bt_simple_nav.xml'])
    ...
    config_bt_nav = PathJoinSubstitution([pkg_bt, 'config', 'bt_nav.yaml'])
    config_planner = PathJoinSubstitution([pkg_bt, 'config', 'planner.yaml'])
    config_controller = PathJoinSubstitution([pkg_bt, 'config', 'controller.yaml'])

    # Include Gazebo Simulation
    # launch_gazebo = IncludeLaunchDescription(
    #     PythonLaunchDescriptionSource(
    #         [get_package_share_directory('gz_example_robot_description'), '/launch', '/sim_robot.launch.py']),
    #     launch_arguments={}.items(),
    # )
    launch_leorover = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [get_package_share_directory('team2_nav'), '/launch', '/leo_rover.launch.py']),
        launch_arguments={}.items(),
    )
